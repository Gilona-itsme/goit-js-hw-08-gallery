...
var modalBtn = document.querySelector('[data-action="close-lightbox"]');
var overlay = document.querySelector('.lightbox__overlay');
galleryContainer.addEventListener('click', openModal);
galleryContainer.insertAdjacentHTML('beforeend', createGalleryMarkup(_galleryItems.default)); // Создание и рендер разметки по массиву данных и предоставленному шаблону.
...
function openModal(event) {
  ...
  window.addEventListener('keydown', openNextImg); // window.addEventListener('keyup', moveRight);
...
function closeModal() {
  ...
  window.removeEventListener('keydown', openNextImg); // window.removeEventListener('keyup', moveRight);
...
function openNextImg(event) {
  var i = _galleryItems.default.findIndex(function (item) {
  ...
  if (event.code === 'ArrowRight') {
    if (i === _galleryItems.default.length - 1) {
      return closeModal();
    }
  ...
  modalImage.src = _galleryItems.default[i].original;
  modalImage.alt = _galleryItems.default[i].description;
}
